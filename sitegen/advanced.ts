...
const dom = div(van.bind(renderPre, TextLine))`),
    p("In this piece of code, we have created an element ", Symbol("dom"), ", whose only child is bound to a ", Symbol("boolean"), " state - ", Symbol("renderPre"), ", which determines whether ", Symbol("dom"), " has a ", Symbol("<pre>"), " or ", Symbol("<div>"), " child element. Inside the child element, the underlying text is bound to a ", Symbol("string"), " state - ", Symbol("text"), ". Whenever the value of ", Symbol("renderPre"), " is toggled, a new version of the DOM tree will be generated, and we will add a new binding from ", Symbol("text"), " state to the text node of the newly created DOM tree."),
    p("Without proper garbage collection implemented, ", Symbol("text"), " state will eventually be bound to many text nodes after ", Symbol("renderPre"), " is toggled many times. All the of bindings, except the most recently added one, are actually obsolete, as they bind the ", Symbol("text"), " state to a text node that is not currently being used. i.e.: disconnected from the document tree. Meanwhile, because internally, a ", Symbol("State"), " object holds reference to all DOM elements that are bound to it, these DOM elements won't be GC-ed by JavaScript runtime, causing ", Link("memory leaks", "https://en.wikipedia.org/wiki/Memory_leak"), "."),
